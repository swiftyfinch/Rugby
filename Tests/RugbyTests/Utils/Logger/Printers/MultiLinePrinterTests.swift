...
final class MultiLinePrinterTests: XCTestCase {
    private var sut: MultiLinePrinter!
...
extension MultiLinePrinterTests {
    func test_canPrint() {
        sut = MultiLinePrinter(standardOutput: standardOutput, maxLevel: .result)
        XCTAssertEqual(LogLevel.allCases.map(sut.canPrint(level:)), [true, false, false])

        sut = MultiLinePrinter(standardOutput: standardOutput, maxLevel: .compact)
        XCTAssertEqual(LogLevel.allCases.map(sut.canPrint(level:)), [true, true, false])

        sut = MultiLinePrinter(standardOutput: standardOutput, maxLevel: .info)
    ...
    func test_full() {
        ...
        XCTAssertEqual(standardOutput.printReceivedInvocations, [
            "⚑ \u{1B}[33m[5m 29s] \u{1B}[0mtest_text"
        ])
    ...
    func test_withoutIcon() {
        ...
        // Assert
        XCTAssertEqual(standardOutput.printReceivedInvocations, ["\u{1B}[33m[6s] \u{1B}[0mtest_text"])
    }
    ...
    func test_shift() {
        ...
        // Assert
        XCTAssertEqual(standardOutput.printReceivedInvocations, ["  ⚑ \u{1B}[33m[5m 29s] \u{1B}[0mtest_text"])
    }
    ...
    func test_shiftAndUnshift() {
        ...
        // Assert
        XCTAssertEqual(standardOutput.printReceivedInvocations, [
            "  ⚑ \u{1B}[33m[29s] \u{1B}[0mtest_text0",
            "    ⚑ \u{1B}[33m[17m 9s] \u{1B}[0mtest_text1",
            "  ⚑ test_text2"
        ])
    }
}
